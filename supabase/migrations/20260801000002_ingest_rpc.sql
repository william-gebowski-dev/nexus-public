-- Migration: RPC atômica para ingestão de snapshots AI.
--
-- Antes desta migration, a ingestão era sequencial (snapshot → models →
-- providers → quotas → requests → incidents) com tratamento de erro
-- individual. Se o snapshot fosse inserido mas a inserção de providers
-- falhasse, o snapshot ficava órfão e a API retornava dados incompletos.
--
-- Agora, `ingest_ai_snapshot` aceita o payload JSON completo, insere
-- tudo em uma única transação, e retorna `run_id` em caso de sucesso.
-- A tabela `ai_ingest_runs` é atualizada com `succeeded` ou `failed`
-- atomicamente.
--
-- Fallback: se a RPC não existir (PGRST202 — deploy antigo), o
-- `api/ai/ingest.ts` já tem lógica de cair para inserts sequenciais
-- com status `partial`. A RPC é a rota preferencial mas não é
-- obrigatória.
...
BEGIN
  -- Verificar idempotência: se a chave já foi processada, retornar erro.
  IF EXISTS (
  ...
  IF jsonb_array_length(p_provider_quotas) > 0 THEN
    INSERT INTO ai_provider_quotas (id, provider_id, provider_name, quota_type, status, used_pct, remaining_pct, resets_at, message)
    SELECT
...
EXCEPTION WHEN OTHERS THEN
  -- Em erro, marcar run como failed e relançar.
  UPDATE ai_ingest_runs
...
COMMENT ON FUNCTION ingest_ai_snapshot IS
  'Ingestão atômica de snapshot AI: insere snapshot + model_usage + provider_usage + '
  'quotas + requests + incidents em uma transação. Em erro, toda a ingestão '
  'é revertida (rollback via RAISE AFTER UPDATE) e o run fica failed.';